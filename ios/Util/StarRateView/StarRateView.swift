...
public class StarRateView: UIView {
  ...
  @IBInspectable public var hightLightImage:UIImage?
  //回调函数
...
extension StarRateView{
  ...
  fileprivate func setupUI(){
    ...
    //背景view
    self.backgroundView = self.creatStarView(image: defaultImage)
    //选择view
    self.foreView = self.creatStarView(image: hightLightImage)
  ...
  fileprivate func setTap() {
    if isSupportTap {
      let tap = UITapGestureRecognizer(target: self, action: #selector(StarRateView.tapStar(sender:)))
