...
class TapLabel:UILabel {
  ...
  private var isTapAction : Bool = false
  ...
  var enabledTapAction : Bool {
    set {
      isTapAction = newValue
    }
    get {
      return isTapAction
    }
  ...
  fileprivate func yb_getTapFrame(_ point : CGPoint , result : ((_ str : String ,_ range : NSRange ,_ index : Int) -> Void)) -> Bool {
    ...
    guard let attrStr = self.attributedText else { return false }
    ...
    for i : CFIndex in 0..<count {
      ...
      if rect.contains(point) {
        ...
        if let link_count = attributeStrings?.count {
  ...
  fileprivate func yb_saveEffectDicWithRange(_ range : NSRange) -> Void {
    ...
    if let subAttribute = self.attributedText?.attributedSubstring(from: range) {
