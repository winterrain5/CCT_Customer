...
class WalletPackagesCell: UITableViewCell,UICollectionViewDelegate,UICollectionViewDataSource {
  var packages:[WalletPackagesModel] = [] {
    didSet {
      self.shouldDisplay = !(packages.count > 0)
  ...
  var emptyString:NSMutableAttributedString = {
    let str = "You have no new Packages"
  ...
  override func awakeFromNib() {
    ...
    collectionView.register(nibWithCellClass: WalletPackageItemCell.self)
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    let cell = collectionView.dequeueReusableCell(withClass: WalletPackageItemCell.self, for: indexPath)
    if packages.count > 0 {
